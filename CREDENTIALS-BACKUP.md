...
| IMAP app password | Android Keystore | Regenerable | Revoke at the provider and issue a new one. |
...
(The Windows desktop app ended at v2.1.5, tag `windows-final`; its dev signing
cert and `sign_exe.ps1` no longer exist in this repo and are not relevant to a
rebuild.)
...
1. **Revoke the IMAP app password** at the mail provider.
