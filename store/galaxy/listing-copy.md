...
> 2. Chat Mail Sync turns the export into email: one message per day of conversation,
>    in date order, with the attachments still attached. They are threaded together, so a
>    long chat arrives as many emails inside a single thread rather than one unopenable
>    giant.
