...
4. **After import, synced files:** *Leave in place* (default) · *Move to a
   "synced" subfolder* · *Delete after import*.

**Interval options:** every 15 min (default) · every 30 min · every
hour · every 3 hours · every 6 hours · every 12 hours · once a day.

**There is no option shorter than 15 minutes.** Android's background scheduler
enforces that as a hard floor. No app of any kind can go under it.

Android checks on its own schedule even with the app closed — that is the whole
point of doing it through the system scheduler.
...
### Rules the watched folder follows
...
is the answer, and a rule separates one from the next. The Android **Help &
FAQ** screen carries the same questions in the same order.
...
your phone. One chat at a time — WhatsApp has no multi-select export — but the app
...
**A.** Encrypted at rest, on the phone you entered it on, with a key held in the
phone's Android Keystore, which never leaves the phone's secure hardware. It is
never written into the app's settings file, never shown back to you in the
password box after saving, and never included in a log line or an error
message.
...
### Q. I set the app up on a new phone, and it wants the password again.

**A.** That is expected, not a fault. The saved app password is encrypted with a
key tied to the Android Keystore on the particular phone it was entered on. Moved
to a new phone, or reinstalled after an uninstall, the app cannot unlock it.
Nobody who picks up the phone can read your password either, which is the
point. Your app password is still valid at your provider: just enter it again
under **Settings → Mail account** on the new device.
...
15 minutes to once a day, where Android's background scheduler imposes the
floor. Section 6 has every option.

---

### Q. The schedule stopped running on its own. Why?

**A.** The app keeps checking with the app closed, so the cause is almost always
battery optimisation: the system has put the app to sleep, and a sleeping app
gets no background ticks. Exempt it once — stock Android, Samsung's sleep lists
and the Autostart permission on Xiaomi/Oppo/Vivo/OnePlus are all covered in
section 6. This does not affect a manual sync, which runs in the foreground
while you watch.
...
While a cutoff is set, a card above the inbox says so permanently, rather than
as a notice you can dismiss. A floor you have forgotten about looks exactly
like an app that is losing your messages.
...
duplicates. **Any** two instances behave this way — two phones, or two separate
installs on the same phone.
...
### Q. I am moving to a new phone. How do I take my history with me?
...
Your mail password is deliberately left out, so the new phone asks for it once.
Restoring **merges** rather than replaces, so a restore onto an app that has
already synced something keeps both sides, and restoring the same backup twice
...
happens on the device can take them out of it. What a reset, a reinstall or a
wiped device destroys is the record of what has already been sent, and without
that record the app mails every chat a second time, into a mailbox that has no
way to tell the copies apart.

The protection is **Settings -> Backup & restore -> Save a backup**. It writes that
record to a file you keep somewhere else — not inside the app's own storage, which
is the thing that goes missing. Keep a recent one; the section tells you when the
last one was taken. Android's own system backup carries the record as well. Your
mail password is in neither, so you enter it once after a restore.
...
**A.** Tap it in the chats list. That opens a screen for that one chat, showing
when it last synced, how many messages have gone out, whether a mail thread
already exists for it and which export file it came from. The same three
actions are on it: sync just that one chat, reset it, or delete it from the
list.

**Sync just this chat** runs a normal sync limited to that chat, instead of
everything waiting in the inbox.

---

### Q. I want to re-do a chat from scratch. What does Reset do?

**A.** Open the chat and choose **Reset (forget sync history)**. This clears the
app's record of that chat and, where it can, moves its file back to the inbox so
you can sync it again. It does **not** delete anything already in your mailbox.
...
**A.** Removing a chat from the list — **Delete from list** — does **not** delete
...
**A.** Yes — use the CSV export from the chat list to get a spreadsheet of all
your synced chats.
